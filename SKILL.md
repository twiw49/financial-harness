...
**헤드리스/배치(비대화형 — `claude -p`·CI·cron) 실행: 질문-종료 금지.** AskUserQuestion이 불가하거나 무응답이면 권장 기본값을 스스로 채택해 진행하고 가정 1줄을 보고서 상단에 명시한다. 데이터 부재·API 갭으로 원 설계가 불가하면 **best-available 대안**(범위 축소·웹/DART 하이브리드 소싱)으로 산출물을 완성하고 한계를 명시한다 — **산출물 0으로 종료하는 것이 실패**이지, 한계 명시한 부분 산출물이 실패가 아니다. (백테스트 STOP 금지와 같은 원칙.) 채택한 기본값이 요청 문언의 **다른 해석을 배제**하면 그 분기와 배제 사유를 정의 절에 1줄 고지한다(조용한 치환 금지).
...
3. **플랜 수령** — `mcp__hyean__get_plan(request_type, context)` 1콜.
   - request_type: **§2.3 결정표**로 분류 (11종). 복합 요청은 §2.3 체인 런 규정.
...
## 2.3 요청 분류 결정표 (request_type 11종)
...
| 산업 내 유망 종목(Top pick) | `sector` | **산출물에 종목 추천이 있으면 sector**; 순수 산업 구조(추천 0)면 `industry_report` |
| 산업 구조·경쟁(누가 크고 누가 진입) | `industry_report` | 종목 추천·목표가가 산출물이면 `sector` |
| 조건식 발굴 | `screening` | 상위 종목 심층까지 원하면 **체인 런**(screening→종목당 `single_stock`, §2.3 하단) |
| 성과 통계·전략 아이디어 검증 | `quant` (모드 A: 사후 성과분해) | **규칙+기간+리밸런스가 명시**되면 **모드 B(백테스트)** — `/datasets` 다운로드 + `scripts/backtest_scaffold.py` 로컬 실행 |
| 보유 목록 진단 / 정기 모니터링(변동 브리핑) | `portfolio` (진단·브리핑 2모드 — _state 있으면 delta, 없으면 full) | — |
| 매크로 지표 전망 | `macro` | — |
