...
- **경량·스캔** (screening·watchlist_brief): 목록/표 중심. **verdict-hero·scenario-bar·종목 심층·투자행동가이드 넣지 말 것.**
  screening=조건충족 목록+지표값+산정정의+위생로그(장문 분석 금지). watchlist=변동 종목 1줄 이유+안정 명단+수급 델타.
- **중량·초점** (event·macro·industry): 핵심 차트 2~4, 초점 유지. event는 **선정한 1건에 한정**(참고 KPI·밸류 방향성 확장 금지).
- **고중량·종합** (single_stock·dd·compare·sector·deep_screening·portfolio·custom·quant): 표+차트 다층, 깊이 필요.
- ★**유형이 명시 배제한 것 금지**: dd·portfolio·industry·screening은 **목표가·매수의견 없음**(verdict-hero를 쓰면 목표가가 아니라 리스크등급·판정으로).
