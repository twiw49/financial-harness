...
- **★표시값(disp) = 인용 data-value 정합 (추적성 계약, 어기면 드로어 클릭 시 화면과 다른 숫자):**
  ...
  - **%지표는 자릿수 관용 없음**: 금액은 '7.98'=7.98조처럼 단위 생략이 정상이지만 %값엔 단위 생략이 없다 —
    표시 %가 인용값과 ×100(비율↔퍼센트) 이외 배율로 어긋나면(예 0.09%↔85.38%) 토큰 미스가 아니라 **데이터
    손상 신호**: 분자÷분모 독립 재구성으로 어느 쪽이 맞는지 검증 후 채택·제외를 결정하라(교정 없이 통과 금지).
...
- **★기준 정합 (as-of·연결/별도·fwd/trailing) — 무라벨 혼용 금지** (블랙박스 채점 최다 감점): 한 표·행·KPI에 배수·시총·수익률·순차입을 나란히 실을 때 **서로 다른 시점·정의를 라벨 없이 섞지 말 것**. 흔한 위반:
  ...
  - **동일 라벨 = 동일 산식(분모까지)**: '총환원율'·'배당성향' 같은 파생 라벨은 보고서 전체에서 단일 정의 —
    (배당+소각)÷순이익 과 ÷시총 은 **별개 지표**이므로 병기하려면 라벨을 갈라라("총환원율(순이익 대비)" /
    "총환원수익률(시총 대비)"). 표·차트 축·요약이 같은 라벨을 다른 분모로 재계산하면 즉시 자기모순.
...
- **요약·알림 ↔ 카드/표 대조**: Executive Summary·브리핑 알림·헤드라인 문장의 카운트·수치·방향은 본문
  카드/표에서 파생하고 발행 전 재대조 — 요약에만 존재하는 수치 금지(알림 문장과 소스 카드가 3중 불일치했던 선례).
- **미보유 데이터 침묵 금지**: 유형 표준 재료(수급·자사주 매입금액 등)가 API에 없으면 해당 섹션에 "데이터
  없음(미수집)"을 명시하고, 웹 폴백 시 출처 표기 — 조용한 누락은 분석 누락과 동급 감점.
